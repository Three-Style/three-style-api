...
module.exports.getOrderController = async (req, res) => {
	...
	const AuthorizedUser = req.headers.userAuthData;
	...
	try {
		const BodySchema = Joi.object()
			...
			.concat(JoiPaginationSchema.keys({ limit: Joi.number().integer().min(1).max(50).optional() }))
		...
		const CacheKey = GetOrderPrefix + AuthorizedUserId + JSON.stringify(Params),
			cacheTTL = 60 * 2;
		...
		let findQuery = {
			user_id: convertToObjectId(AuthorizedUserId),
		};
		...
		const SearchFields = ['_id', 'receipt_id', 'gateway_order_id', 'gateway_transaction_id', 'order_item_id', 'order_item_type', 'invoice_id', 'multiple_items[].item_id', 'multiple_items[]._id'];
