...
module.exports = async (req, res) => {
	req.logger.info('Controller > Admin > Sub Categories > Get Sub Categories');
	...
	try {
		...
		const CountDocs = await SubCategoriesRepo.countDocuments(findQuery);
		const PaginationInfo = PaginationHelper.getPaginationInfo(CountDocs, req.query);

		// DB: Find
		return SubCategoriesRepo.find(findQuery)
