...
module.exports = async (req, res) => {
	req.logger.info('Controller > Admin > Fabric > Get Fabric');
	...
	try {
		...
		const CountDocs = await FabricRepo.countDocuments(findQuery);
		const PaginationInfo = PaginationHelper.getPaginationInfo(CountDocs, req.query);

		// DB: Find
		return FabricRepo.find(findQuery)
